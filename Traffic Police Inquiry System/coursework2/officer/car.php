...
<body>
	...
	<p align="center"><input class="add" type="button" value="Add" name="inbut" onClick="location.href='http://mersey.cs.nott.ac.uk/~alysg10/coursework2/officer/vehicleadd.php'" />
	<input type="text" name="sel" /><input type="submit" value="Search" name="selsub" /></p>
...
<?php      
	...
	if (empty($_POST["sel"])){
		...
		FROM Vehicle t1 left join People t2 on t1.P_ID=t2.People_ID order by t1.Vehicle_ID;");
	}else{
		$sel = $_POST["sel"];
		$res = mysqli_query($link,"SELECT t1.Vehicle_ID,t1.Vehicle_type,t1.Vehicle_colour,t1.Vehicle_licence,t2.People_name,t2.People_licence 
		FROM Vehicle t1 left join People t2 on t1.P_ID=t2.People_ID WHERE t1.Vehicle_licence like '%$sel%';"); 
	}
